...
The simulations were in agreement with the experiments, both quanlitatively respect to the precense of stagnation and re-acceleration, and quantitatively with respect to the stagnation velocity and bubble height at the onset of re-acceleration.
...
The Boussinseq approximation, low Schmidt number approximation, and linear initial condition approximations preserve the re-acceleration phenomena.
More generally no physical processes beyond the incompressible Navier-Stokes with an active scalar are nessesary to stagnation and re-acceleration, so they can be studied with NekBox.
...
To experimentally access the late-time dynamics of periodic arrays of bubbles requires techiques to drive initial perturbations with many more modes.
